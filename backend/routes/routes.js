...
router.delete('/:id/schedules/:scheduleId', async (req, res) => {
  try {
    ...
    await db.query(`DELETE FROM route_schedules WHERE id = ? AND route_id = ?`, [scheduleId, routeId]);
    res.sendStatus(204);
  } catch (err) {
...
router.get('/:id/stations', async (req, res, next) => {
  try {
    ...
    const reordered = isReturnDirection(finalDirection)
      ? rows.slice().reverse().map((row, idx) => ({ ...row, sequence: idx + 1 }))
      : rows;

    res.json(reordered.map((row) => ({ ...row, direction: finalDirection })));
  } catch (err) { next(err); }
});
...
router.get('/price', async (req, res) => {
  const { route_id, from_station_id, to_station_id, category, date, route_schedule_id } = req.query;
